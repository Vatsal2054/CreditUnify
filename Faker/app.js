...
async function scrapeLoanData(loanType) {
  if (!URLS[loanType]) {
    return { error: `Invalid loan type. Choose from: ${Object.keys(URLS).join(', ')}` };
  }
  ...
  try {
    ...
    tables.each((tableIndex, tableElement) => {
      ...
      if (theadHeaders.length > 0) {
        ...
        const firstRowCells = $(tableElement).find('tr:first-child td, tr:first-child th');
      ...
      const relevantHeaders = ['Bank', 'Interest Rate', 'Tenure', 'Name of the Bank', 'Interest Rate (p.a.)'];
      const isLoanTable = headers.some(header => relevantHeaders.some(relevantHeader => header.includes(relevantHeader)));
...
app.get("/get-scores", (req, res) => {
  ...
  Object.entries(bureaus).forEach(([bureau, range]) => {
    if (Math.random() < 0.2) {
      creditScores[bureau] = "Credit Score Not Available";
    } else {
      creditScores[bureau] = faker.number.int({
        min: range.min,
        max: range.max,
      });
    }
  ...
  res.status(200).json({
    ...
    accountSummary: {
      totalActiveAccounts: faker.number.int({ min: 1, max: 5 }),
      totalClosedAccounts: faker.number.int({ min: 0, max: 3 }),
      outstandingDebt: faker.number.float({ min: 10000, max: 1000000, precision: 2 }).toFixed(2),
      recentEnquiries: generateEnquiries(),
    },
    paymentHistory: Array.from({ length: 6 }, (_, i) => ({
      month: faker.date
        .past({ years: 1, months: i })
        .toISOString()
        .split("T")[0]
        .slice(0, 7),
  ...
  res.send(`
    <html>
        ...
        <body>
            ...
            <ul>
                <li><strong>type</strong> (required): Type of loan data to fetch. Options: ${Object.keys(URLS).join(', ')}</li>
